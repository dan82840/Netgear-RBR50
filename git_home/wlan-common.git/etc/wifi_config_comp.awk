...
function parse_wireless_item(items, status_str){
    ...
    for (i in wireless_type){
        if (item[2] == i || item[2] == wireless_type[i]){
    ...
    # Check if there is any change related to wifi device setting
    if (item[2] == wifi_device_name[0] || item[2] == wifi_device_name[1] || item[2] == wifi_device_name[2]){
    ...
    }else if (match(item[2], "wl[ga]") || match(item[2], "wifi-iface")){
...
END {
    ...
    for (dev_index in wifi_device_name){
        if (wifi_device_change[wifi_device_name[dev_index]] != "" ){
            wifi_device_whole_change = wifi_device_change[wifi_device_name[dev_index]]
        } else
            wifi_device_whole_change = 0
