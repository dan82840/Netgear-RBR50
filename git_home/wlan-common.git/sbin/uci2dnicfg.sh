...
set_vap_ssid()
{
	...
	echo "$ssid" > /tmp/yh_ssid
	sed -i 's/\\/\\\\\\\\/g' /tmp/yh_ssid
	sed -i 's/"/\\\"/g' /tmp/yh_ssid
	sed -i 's/`/\\\\\\\`/g' /tmp/yh_ssid


    [ -n "$ssid" ] && {
        dni_config_set ${_prefix}ssid "`cat /tmp/yh_ssid`" 
    }
	rm -f /tmp/yh_ssid
}
...
set_vap_security()
{
    ...
    case "$encryption" in
        ...
        psk2*)
			...
			echo "$key" > /tmp/yh_key
			sed -i 's/\\/\\\\/g' /tmp/yh_key
			sed -i 's/"/\\\"/g' /tmp/yh_key
			sed -i 's/`/\\\`/g' /tmp/yh_key
            dni_config_set ${_prefix}wpa2_psk "`cat /tmp/yh_key`" 
			rm -f /tmp/yh_key
            ;;
        *mixed*)
			...
			sed -i 's/"/\\\"/g' /tmp/yh_key
			sed -i 's/`/\\\`/g' /tmp/yh_key
            dni_config_set ${_prefix}wpas_psk "`cat /tmp/yh_key`" 
			rm -f /tmp/yh_key
            ;;
        psk*)
			...
			sed -i 's/"/\\\"/g' /tmp/yh_key
			sed -i 's/`/\\\`/g' /tmp/yh_key
            dni_config_set ${_prefix}wpa1_psk "`cat /tmp/yh_key`"
			rm -f /tmp/yh_key
            ;;
    esac
}
