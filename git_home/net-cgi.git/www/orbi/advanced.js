...
function subItemsClass(argv)
{
	...
	for(num=0; num<arguments.length; num++)
	{
		...
		for(i=0; i<items.length; i++)
		{
			...
			words_height = items[i].getElementsByTagName("span")[0].clientHeight;
			if(words_height > 30 )
				items[i].className = "long_name";
			else if( words_height > 16 )
...
var ap_items = [ "internet", "wan", "block_site", "block_services", "forwarding_triggering","vpn", "remote", "upnp", "traffic", "wl_bridge", "lan", "ipv6", "parental", "access_control", "usb_set", "devname"];
