...
function firmwareUpgrade2()
{
	goto_formframe('go_upgrade.htm');
}
...
function have_new_satellite_ver()
{
	for(var i=0, len=ext_check_info.length; i<len; i++)
	{
		if(ext_check_info[i]['check status'] == "000" && ext_check_info[i]['new version'] != "0")
		{
			for(var j=0, len2=ext_dev_info.length; j<len2; j++)
			{
				if(ext_dev_info[j]['mac address'] != ext_check_info[i]['mac address'])
					continue;
				else if(ext_dev_info[j]['current version'] == ext_check_info[i]['current version'])
					return 1;
				else
					break;
			}
...
function load_top_value()
{
	...
	var have_new_satellite = have_new_satellite_ver();
	if(upgrade_div != null)
	{
		if(wan_status==1 && ((config_status==9999 || config_status == 10004)&& updateFirmware==1 || have_new_satellite==1))
	...
	if( upgrade_div.style.display != "none")
	{
		...
		var info_width = document.getElementById("update_info").clientWidth;
