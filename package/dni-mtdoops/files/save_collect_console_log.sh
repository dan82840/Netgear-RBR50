...
if [ "x$collect_log" = "x1" ];then
	zip debug-log.zip  NETGEAR_$module_name.cfg panic_log.txt Console-log1.txt Console-log2.txt basic_debug_log.txt wireless-log1.txt wireless-log2.txt lan.pcap wan.pcap soapclient/* /var/log/soapclient/* hyt_result satellite_status hyd-restart.log wsplcd-restart.log $RADARLOG
else
	zip debug-log.zip NETGEAR_$module_name.cfg  panic_log.txt  Console-log1.txt Console-log2.txt wireless-log1.txt wireless-log2.txt basic_debug_log.txt lan.pcap wan.pcap soapclient /var/log/soapclient/* hyt_result satellite_status hyd-restart.log wsplcd-restart.log $RADARLOG
fi

cd /tmp
rm -rf debug-usb debug_cpu debug_flash debug_mem debug_mirror_on debug_session NETGEAR_$module_name.cfg panic_log.txt Console-log1.txt Console-log2.txt basic_debug_log.txt lan.pcap wan.pcap wireless-log1.txt wireless-log2.txt
