...
UCLIBC_MAKE = PATH='$(TOOLCHAIN_DIR)/initial/bin:$(TARGET_PATH)' $(MAKE) $(HOST_JOBS) -C $(HOST_BUILD_DIR) \
	...
	ARCH="$(CONFIG_ARCH)" \
	LIBGCC="$(subst libgcc.a,libgcc.a,$(shell $(TARGET_CC) -print-libgcc-file-name))" \
	DOSTRIP=""
...
define Host/Configure
	...
	$(SED) 's,^KERNEL_HEADERS=.*,KERNEL_HEADERS=\"$(BUILD_DIR_TOOLCHAIN)/linux-dev/include\",g' \
		...
		-e 's,^.*UCLIBC_HAS_SSP[^_].*,UCLIBC_HAS_SSP=$(if $(CONFIG_DISABLE_HARDENING),n,y),g' \
		-e 's,^.*UCLIBC_HAS_SSP_COMPAT.*,UCLIBC_HAS_SSP_COMPAT=$(if $(CONFIG_DISABLE_HARDENING),n,y),g' \
		-e 's,^.*SSP_QUICK_CANARY.*,SSP_QUICK_CANARY=$(if $(CONFIG_DISABLE_HARDENING),n,y),g' \
		-e 's,^.*PROPOLICE_BLOCK_SEGV.*,PROPOLICE_BLOCK_SEGV=$(if $(CONFIG_DISABLE_HARDENING),n,y),g' \
		-e 's,^.*UCLIBC_BUILD_NOW.*,UCLIBC_BUILD_NOW=$(if $(CONFIG_DISABLE_HARDENING),n,y),g' \
		-e 's,^.*UCLIBC_BUILD_PIE.*,UCLIBC_BUILD_PIE=$(if $(CONFIG_DISABLE_HARDENING),n,y),g' \
